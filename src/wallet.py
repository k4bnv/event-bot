"""Per-strategy virtual wallet.

Each strategy owns its own isolated slice of the $100 demo capital. OKX's
Demo Trading account itself is a single shared balance — it has no concept
of "sub-wallets per strategy" — so wallet isolation is implemented entirely
inside the bot. This is what lets strategy A, B and C run "independently"
against the same OKX market data feed while never touching each other's
capital or being able to over-spend it.
"""
