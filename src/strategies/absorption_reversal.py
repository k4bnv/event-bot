...
def book_side_replenished(
    ...
    """True if the resting `side` ("bid" or "ask") hasn't materially
    drained over the last `lookback_sec` — current volume is still at
    least `min_ratio` of what it was back then. `history` must be
    chronological (oldest first — see market_data.py's
    btc_orderbook_history/_record_orderbook). False (never "confirmed")
    on too little history — an absorption signal with no real
    confirmation data available is not a confirmed one."""
    if len(history) < 2:
        return False
    ...
    if past_vol <= 0:
        return False
    return current_vol >= past_vol * min_ratio
...
class AbsorptionReversalStrategy(BaseStrategy):
    ...
    async def evaluate(self, ctx: StrategyContext) -> Optional[Signal]:
        ...
        if not book_side_replenished(
            list(ctx.orderbook_history), now, confirm_lookback_sec, book_depth, confirm_side, min_replenish_ratio,
        ):
            return None
        ...
        if not breaks_local_range(trigger_points, candidate):
            return None
